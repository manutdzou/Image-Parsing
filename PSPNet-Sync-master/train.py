...
def main():
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        with tf.name_scope('PSPnet') as scope:
            ...
            net = PSPNet({'data': images_splits}, is_training=True, num_classes=args.num_classes)
    
            raw_output_list = net.layers['conv6']

            add_list = ['conv5_3_pool1_conv', 'conv5_3_pool1_conv_bn', 'conv5_3_pool2_conv', 'conv5_3_pool2_conv_bn', 'conv5_3_pool3_conv', 'conv5_3_pool3_conv_bn', 'conv5_3_pool6_conv','conv5_3_pool6_conv_bn', 'conv5_4', 'conv5_4_bn', 'conv6']
        ...
        restore_var = [v for v in tf.trainable_variables() if 'conv5_3_pool' not in v.name and 'conv5_4' not in v.name and 'conv6' not in v.name]+[v for v in tf.global_variables() if ('moving_mean' in v.name or 'moving_variance' in v.name) and ('conv5_3_pool' not in v.name and 'conv5_4_bn' not in v.name and 'biased' not in v.name and 'local_step' not in v.name)]
