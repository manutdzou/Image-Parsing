...
class Network(object):
    ...
    def batch_normalization(self, input, name, relu=False):
        ...
        x_shape = input.get_shape().as_list()
        params_shape = x_shape[-1:]
        axis = list(range(len(x_shape) - 1))

        mean = tf.reduce_mean(input, axis)
        variance = tf.reduce_mean(tf.square(input), axis) - tf.square(mean)
        ...
        tf.add_to_collection(tf.GraphKeys.UPDATE_OPS, moving_averages.assign_moving_average(moving_mean, mean, decay, zero_debias=False))
        tf.add_to_collection(tf.GraphKeys.UPDATE_OPS, moving_averages.assign_moving_average(moving_variance, variance, decay, zero_debias=False))
    
        output = tf.cond(self.is_training, lambda:tf.nn.batch_normalization(input, mean, variance, beta, gamma, epsilon), lambda:tf.nn.batch_normalization(input, moving_mean, moving_variance, beta, gamma, epsilon))
