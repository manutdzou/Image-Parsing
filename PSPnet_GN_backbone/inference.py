...
tf.app.flags.DEFINE_integer('num_steps', 2000001, 'Number of step.')
...
tf.app.flags.DEFINE_integer('gpu_nums', 2,
                            'Number of gpus used for training')
tf.app.flags.DEFINE_float('learning_rate', 0.01, 'lr.')
...
RMSPROP_EPSILON = 1                # Epsilon term for RMSProp.
...
class Model_Graph(object):
    ...
    def _build_defaut_graph(self, images):
        ...
        Densenet
        """
        model = model_zoo.PSPnet_backbone(num_class = self.num_class,
...
def main(_):
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        train_op = tf.group(opt.apply_gradients(grads, global_step), variables_averages_op)
